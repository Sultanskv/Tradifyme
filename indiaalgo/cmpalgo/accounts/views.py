...
def send_trade_to_angel_broking(trade, trade_type, symbol, price, auth_token, qty, client_id, api_key, SYMBOL_m):
    try:
        ...
        if not symbol_token:
            logger.error(f"Invalid symbol token for symbol: {symbol}")
            symbol_token = "Unknown"  # Placeholder in case symbol_token is invalid
        ...
        # Save trade data to Trade model immediately after order placement attempt
        trade_record = Trade(
        ...
        if not client_user.Term_condition:
            ...
            try:
                ...
                message = (
                    ...
                    "We would like to inform you that all subscription fees paid to 'Tradifyme' are non-refundable. "
                    ...
                    "does not indicate future performance of any current or future strategy, model, or advice provided by 'Tradifyme'.\n\n"
                    ...
                    "objectives of any strategy, model, or advice provided by 'Tradifyme' will be achieved.\n\n"
                    
                    "'Tradifyme', its partners, principal officers, and employees do not assure or guarantee any return on the investment "
                    "in strategies, models, or advice given to the investor. The value of investments can fluctuate depending on various "
                    "market factors and forces. 'Tradifyme' or its associates are not liable or responsible for any loss or shortfall arising "
...
def authenticate_client(client_id):
    client_data = ind_clientDT.objects.get(clint_id=client_id)
    client = SmartConnect(api_key=client_data.api_key)
    client.setAccessToken(client_data.auth_token)
    # Optionally handle token refresh logic here if necessary
    return client

def fetch_and_store_orders(client_id):
    client = authenticate_client(client_id)
    
    try:
        order_book = client.orderBook()
        print(f"Order Book Data: {order_book}")  # Debug: Print the response
        for order_data in order_book['data']:
            print(f"Processing Order: {order_data}")  # Debug: Print each order data
            order_id = order_data['orderid']
            symbol = order_data['tradingsymbol']
            quantity = order_data['quantity']
            price = order_data['price']
            status = order_data['status']
            reject_reason = order_data.get('reason', '')
        ...
        print(f"Error fetching orders: {e}")  # Debug: Print any errors
...
from django.contrib import messages
from .models import ind_clientDT
# from smartapi import SmartConnect  # Import the SmartConnect class from the Angel API package 
def fetch_and_display_orders(request):
    if 'clint_id' in request.session:
        user_id = request.session.get('clint_id')
        try:
            client_user = ind_clientDT.objects.get(clint_id=user_id)
        except ind_clientDT.DoesNotExist:
            messages.error(request, "Client user not found")
            return redirect('/')
        ...
        try:
            # Initialize the SmartConnect client
            client = SmartConnect(api_key=api_key)
            client.setAccessToken(auth_token)

            # Fetch the order book
            order_book_response = client.orderBook()

            # Check if orders are fetched correctly
            if 'data' in order_book_response and isinstance(order_book_response['data'], list):
                orders = order_book_response['data']
                
                # Loop through the orders and save them to the database
                for order_data in orders:
                    # Check if order already exists
                    existing_order = Order.objects.filter(orderid=order_data['orderid']).first()
                    if not existing_order:
                        ...
                        Order.objects.create(
                            ...
                            averageprice=float(order_data['averageprice']),
                            transactiontype=order_data['transactiontype'],
                            updatetime=order_data['updatetime'],
                            text=order_data.get('text', '')
                        )

                # Pass the orders to the template for display
                return render(request, 'order_status.html', {'orders': orders, 'client_user': client_user})
            else:
                messages.error(request, "No orders found")
                return render(request, 'order_status.html', {'orders': None, 'client_user': client_user})

        except Exception as e:
            messages.error(request, f"Error in fetching order status: {str(e)}")
            return redirect('/')
